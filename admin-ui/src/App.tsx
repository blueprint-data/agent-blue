          ...
          {loading ? (
            ...
            <label className="tenant-select-label">
              ...
              <select
                ...
                disabled={!isSuperadmin && Boolean(scopedTenantId)}
                onChange={(event) => {
                  ...
                  if (!isSuperadmin && scopedTenantId && v !== scopedTenantId) {
                    return;
                  }
                  setSelectedTenantId(v);
                }}
              >
                {isSuperadmin ? <option value="__new__">New tenant…</option> : null}
                {tenants.map((tenant) => (
                  <option key={tenant.tenantId} value={tenant.tenantId}>
                    {tenant.tenantId}
                    {tenant.repoUrl ? ` — ${compactText(tenant.repoUrl, 48)}` : ""}
                  </option>
                ))}
              </select>
            </label>
